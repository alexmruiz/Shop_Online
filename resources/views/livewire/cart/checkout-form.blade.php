<div class="mt-4 mb-4">
    <x-card cardTitle="Finalizar compra">
        <div>
                ...
                <form wire:submit.prevent="processPayment">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" id="street" wire:model.defer="street" class="form-control" placeholder="Calle, número, piso" required/>
                            @error('street') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="postalCode" class="form-label">Código Postal</label>
                            <input type="text" id="postalCode" wire:model.defer="postalCode" class="form-control" placeholder="Ej: 28001" required />
                            @error('postalCode') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="city" class="form-label">Ciudad</label>
                            <input type="text" id="city" wire:model.defer="city" class="form-control" placeholder="Ej: Madrid" required/>
                            @error('city') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="province" class="form-label">Provincia</label>
                            <input type="text" id="province" wire:model.defer="province" class="form-control" placeholder="Ej: Madrid" required/>
